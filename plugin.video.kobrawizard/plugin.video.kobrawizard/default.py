...
H = 'http://'
EXCLUDES     = ['plugin.video.kobrawizard','script.module.addon.common','script.kobra.builds','plugin.program.kobra.custompack.installer','repository.kobra']
...
def INDEX():
    ...
    addDir('CONTACT KOBRA BUILDS',BASEURL,8,ART+'facebook.png',FANART,'')
    setView('movies', 'MAIN')
...
def facebook():
    TextBoxes('Kobra Wizard Updates', '[COLOR=steelblue]Welcome to Kobra Builds. Kobra H24 Build. For support please join our facebook group Kodi Custom Builds. [B]https://www.facebook.com/groups/KodiCustomBuilds/[/B]. You Tube Channel [B]	https://www.youtube.com/channel/UCylDOZ_hQ7zPBoWE9gy28CA[/B][/COLOR]')
...
def WIZARD(name,url,description):
    ...
    dp.create("Kobra Wizard","Downloading your build ",'', 'Please Wait')
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("Kobra Wizard", "To save changes you now need to force close Kodi, Press OK to force close Kodi")
    killxbmc()
        ...
        # Count files and give option to delete
            if file_count > 0:
                ...
                if dialog.yesno("Delete Package Cache Files", str(file_count) + " files found", "Do you want to delete them?"):
                    ...
                    dialog = xbmcgui.Dialog()
                    dialog.ok("Kobra Wizard", "Packages Successfuly Removed", "[COLOR steelblue]Brought To You By Kobra Wizard[/COLOR]")
    except: 
        dialog = xbmcgui.Dialog()
        dialog.ok("Kobra Wizard", "Sorry we were not able to remove Package Files", "[COLOR steelblue]Brought To You By Kobra Wizard[/COLOR]")
...
def deletecachefiles(url):
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("Kobra Wizard", " All Cache Files Removed", "[COLOR steelblue]Brought To You By Kobra Wizard[/COLOR]")
...
def FRESHSTART(params):
    ...
    if yes_pressed:
        ...
        try:
            ...
            else: plugintools.log("freshstart.main_list User files partially removed"); plugintools.message(AddonTitle,"The process is complete, you're now back to a fresh Kodi configuration with Kobra Wizard","Please reboot your system or restart Kodi in order for the changes to be applied.")
